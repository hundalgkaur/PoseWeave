...
**Routes:** splash `/` → onboarding `/onboarding` (first launch) → login `/login` → home `/home`; plus `/camera` `/gallery` `/skeleton3d` `/gait` `/segments` `/image` `/settings` `/profile` `/forgot-password`; clinical `/clinical/login` `/clinical/consent` `/clinical/home` `/clinical/live` `/clinical/biomechanical` `/clinical/3d` `/clinical/gait`.
