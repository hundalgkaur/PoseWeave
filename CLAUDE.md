...
**Routes:** splash `/` → onboarding `/onboarding` (first launch) → login `/login` → home `/home`; plus `/camera` `/gallery` `/skeleton3d` `/gait` `/segments` `/image` `/settings` `/profile` `/forgot-password`.
