...
class PoseClassifierEvent with _$PoseClassifierEvent {
  ...
  /// A recorded/picked clip finished analyzing (`PoseVideoComplete`).
  const factory PoseClassifierEvent.analyzeVideoPoses(List<PoseEntity> poses) =
      ClassifierAnalyzeVideo;
}
...
class PoseClassifierState with _$PoseClassifierState {
  ...
  /// A clip's dominant matched pose (the record-and-match summary).
  const factory PoseClassifierState.summary(PoseMatchResult result) =
      PoseClassifierSummary;
}
...
class PoseClassifierBloc
  ...
  PoseClassifierBloc() : super(const PoseClassifierState.idle()) {
    ...
    on<ClassifierAnalyzeVideo>(_onVideo);
  }
  ...
  void _onVideo(
    ClassifierAnalyzeVideo event,
    Emitter<PoseClassifierState> emit,
  ) {
    emit(PoseClassifierState.summary(PoseClassifier.summarize(event.poses)));
  }
}
